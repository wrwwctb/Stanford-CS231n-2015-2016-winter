...
if __name__ == "__main__":
    ...
    model.add(Conv2D(filters=50, kernel_size=2, strides=(2, 2),
                     padding='same', data_format='channels_first',
                     input_shape=(3, 10, 10),
                     kernel_regularizer=keras.regularizers.l2(regds)))
    model.add(Activation('relu'))
    model.add(Conv2D(filters=100, kernel_size=5, strides=(1, 1),
    ...
    filters0 = model.layers[0].get_weights()[0]
    logistic.plot_filters(filters0, path_results, output_name+'_0', 10)
    filters2 = model.layers[2].get_weights()[0]
    logistic.plot_filters(filters2, path_results, output_name+'_2', 10)
